(ns sculpt.cosci-test
  (:require [clojure.test :refer [deftest testing is]]
            [sculpt.cosci :as cosci]))

(defn- result [id status & {:keys [format bytes safety] :or {format "glb" bytes (byte-array [1]) safety false}}]
  {:candidate {:candidate/id id :prompt (str "prompt-" id) :gene {:subject "s" :category "c"}}
   :status status :artifact-bytes (if (= status :done) bytes nil)
   :format (when (= status :done) format) :expected-format "glb" :safety-flag safety})
...
(deftest run-round-evolution-advances-round
  (let [{:keys [evolution]} (cosci/run-round [(result "a" :done)] {"a" {:score 80}} 5)]
    ...
    (is (= {:subject "s" :category "c"} (:elite-gene evolution)))))
