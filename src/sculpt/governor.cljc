(ns sculpt.governor
  ...
  — not a semantic quality judge, sculpt.cosci already ran that; this
  re-checks hard invariants sculpt.judge/sculpt.cosci have no notion of:
...
(def allowed-formats #{"glb" "gltf" "vrm"})
...
(defn violations
  ...
  sculpt.loop before this check) -> violation strings. Empty = passes."
  [asset]
  (cond-> []
