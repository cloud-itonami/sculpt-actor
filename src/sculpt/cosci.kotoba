(ns sculpt.cosci
  ...
  model candidates (ADR-2607123000 §3), ported-in-shape from
  ...
  slightly-worse asset, it's a disqualified one.
    ...
    Generation  -> sculpt.generate/round-candidates (closed gene pool)
    Reflection  -> hard gates: job actually :done, artifact fetched,
                   declared format matches, no upstream safety flag
    Ranking     -> Elo on sculpt.judge scores (pairwise, draw-tolerant)
...
(defn reflect
  ...
  :format \"glb\" :expected-format \"glb\" :safety-flag bool}."
...
(defn rank
  ...
  missing entries when sculpt.judge/score! failed) -> ranked vector,
...
(defn evolve-round
  "ranked -> {:next-round n+1 :elite-gene {...}} for sculpt.generate's next
