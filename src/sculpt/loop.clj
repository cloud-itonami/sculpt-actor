(ns sculpt.loop
  ...
  through sculpt.cosci -> sculpt.governor -> commit+publish. `run!` reduces
  ...
  queue — see sculpt.murakumo's docstring."
  (:require [clojure.java.io :as io]
            [clojure.edn :as edn]
            [clojure.string :as str]
            [sculpt.persona :as persona]
            [sculpt.generate :as generate]
            [sculpt.murakumo :as murakumo]
            [sculpt.judge :as judge]
            [sculpt.cosci :as cosci]
            [sculpt.governor :as governor]
            [sculpt.datalad :as datalad]
            [sculpt.ledger :as ledger]
            [sculpt.publish :as publish]
            [sculpt.aozora :as aozora]))

(def lease-path ".sculpt/lease")
(def state-path ".sculpt/state.edn")
...
(defn- fetch-result
  "One pending entry -> a sculpt.cosci/reflect-shaped result map, or nil if
...
(defn- asset-title [gene]
  (str/join ", " (remove nil? [(:subject gene) (:category gene)])))
   ...
   (let [p (persona/load-persona)
     ...
     (when winner
       (let [{:keys [result score]} winner
             ...
             asset {:id (str "sculpt-" (str/replace job-id #"[^a-zA-Z0-9-]" "-"))
